    ...
    while true; do
        MAIN=$(
            "${DIALOG[@]}" \
                ...
                --ok-label 'Select' \
                --cancel-label 'Exit' \
                --menu "$NAVIGATION_HINT" -1 -1 0 1 "Patch App" 2 "Update Assets" 3 "Change Source" 4 "Configure" 5 "Fetch Gmscore" 6 "Delete Assets" 7 "Delete Apps" 8 "Specs & Changelog" "${MENU_ENTRY[@]}" \
                2>&1 > /dev/tty
        ) || break
        case "$MAIN" in
            1)
