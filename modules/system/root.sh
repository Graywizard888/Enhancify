...
getInstalledVersion() {
    ...
    elif [ "$RISH_ACCESS" = true ] && ( rish_c "pm list packages --user current | grep -q $PKG_NAME" || ! rish_c "dumpsys package $PKG_NAME" 2>&1 | grep -q "Unable to find package:" ); then
        INSTALLED_VERSION=$(rish_c "dumpsys package $PKG_NAME" | sed -n '/versionName/s/.*=//p' | sed -n '1p')
    fi
}
