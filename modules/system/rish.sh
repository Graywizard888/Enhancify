...
runDexOptimization() {
    ...
    if [ "$INSTALL_TYPE" == "new" ]; then
        PROFILE_MODE="speed-profile"
        PROFILE_NAME="Speed-Profile (Balanced)"
    elif [ "$INSTALL_TYPE" == "update" ]; then
        PROFILE_MODE="speed"
        PROFILE_NAME="Speed (Maximum Performance)"
    else
        PROFILE_MODE="speed-profile"
        PROFILE_NAME="Speed-Profile (Default)"
    fi
    ...
    log "Executing: cmd package compile -m $PROFILE_MODE -f $PKG"
    local OPT_OUTPUT
    OPT_OUTPUT=$(rish -c "cmd package compile -m $PROFILE_MODE -f $PKG" 2>&1)
    local OPT_EXIT_CODE=$?
