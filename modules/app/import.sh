...
selectFile() {
    ...
    while [ ! -f "$newPath" ]; do
        ...
        # Handle "Directory Empty" selection
...
extractMeta() {
    ...
    if [[ "${FILE_PATH,,}" == *.apk ]]; then
        ...
        if ! APP_INFO=$(./bin/aapt2 dump badging "$FILE_PATH" 2>/dev/null); then
            notify error "The APK you selected is not valid. Download again and retry."
            return 1
        fi
        APP_EXT="apk"
        ...
        if ! APP_INFO=$(unzip -qqp "$FILE_PATH" info.json 2>/dev/null); then
            notify error "The Bundle you selected is not valid. Download again and retry."
            return 1
        fi
        if jq -e --arg ARCH "$ARCH" '.arches | index($ARCH) == null' <<< "$APP_INFO" &>/dev/null; then
            notify error "The selected Apk Bundle doesn't contain $ARCH lib.\nChoose another file."
            return 1
        fi
        APP_EXT="apkm"
        source <(jq -rc '
