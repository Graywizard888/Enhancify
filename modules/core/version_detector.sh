...
getVersionUrl() {
    local VERSION="$1"
    local CACHE_FILE CACHED_URL FIRST_URL SELECTED_VERSION_FORMATTED FINAL_URL
    local safe_name="${APKMIRROR_APP_NAME//[^a-zA-Z0-9_-]/_}"
    
    CACHE_FILE="${CACHE_DIR}/${safe_name}_metadata.json"
    
    if [[ -f "$CACHE_FILE" ]]; then
        CACHED_URL=$(jq -r --arg VER "$VERSION" '
            .versions[] | select(.version == $VER) | .url // empty
        ' "$CACHE_FILE" 2>/dev/null)
        
        if [[ -n "$CACHED_URL" && "$CACHED_URL" != "null" ]]; then
            FINAL_URL="https://www.apkmirror.com${CACHED_URL}"
    ...
    if [[ -n "$FIRST_URL" ]]; then
        ...
        FINAL_URL=$(echo "https://www.apkmirror.com${FIRST_URL}" | sed -E "s/[0-9]+-[0-9]+-[0-9]+(-[0-9]+)?-release/${SELECTED_VERSION_FORMATTED}-release/")
    ...
    local VERSION_FORMATTED="${VERSION//./-}"
    echo "https://www.apkmirror.com${APKMIRROR_APP_URL}${APKMIRROR_APP_NAME}-${VERSION_FORMATTED}-release/"
}
