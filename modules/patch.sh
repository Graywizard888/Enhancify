...
initSystemInfo() {
    ...
    _SYS_JAVA_VER=$(awk -F '"' '/version/ {print $2}' <<< "$RAW_JAVA" | cut -d. -f1)
    _SYS_JAVA_VER="${_SYS_JAVA_VER//[!0-9]/}"
    [[ -z "$_SYS_JAVA_VER" ]] && _SYS_JAVA_VER=0

    local FREE_OUT
    FREE_OUT=$(free -m 2>/dev/null)
    _SYS_TOTAL_MEM=$(awk '/^Mem:/{print $2}' <<< "$FREE_OUT")
    _SYS_AVAIL_MEM=$(awk '/^Mem:/{print $7}' <<< "$FREE_OUT")
    if [[ -z "$_SYS_AVAIL_MEM" || "$_SYS_AVAIL_MEM" -eq 0 ]] 2>/dev/null; then
        _SYS_AVAIL_MEM=$(awk '/^Mem:/{print $4}' <<< "$FREE_OUT")
    fi
    ...
    case "$ARCH_RAW" in
        ...
        x86_64)                      _SYS_DEVICE_ARCH="x86_64"      ;;
        x86)                         _SYS_DEVICE_ARCH="x86"         ;;
        *)                           _SYS_DEVICE_ARCH="arm64-v8a"   ;;
    esac
}
...
calculateHeapSize() {
    local HEAP=$((_SYS_AVAIL_MEM * 75 / 100))
    (( HEAP < MIN_HEAP )) && HEAP=$MIN_HEAP
    echo "$HEAP"
}

getMaxHeap() {
    local MAX=$((_SYS_AVAIL_MEM * 75 / 100))
    (( MAX < MIN_HEAP )) && MAX=$MIN_HEAP
    echo "$MAX"
}
...
buildParallelGCArgs() {
    local HEAP_SIZE="$1"
    local INITIAL_HEAP=$((HEAP_SIZE / 3))
...
patchApp() {
    ...
    local HEAP_SIZE MAX_HEAP
    HEAP_SIZE=$(calculateHeapSize)
    MAX_HEAP=$(getMaxHeap)

    local GC_MODE="G1 Engine" GC_INFO="G1GC (Default)"
