...
checkJavaVersion() {
    ...
    if [ "$JAVA_VER" -eq 17 ] || [ "$JAVA_VER" -eq 21 ] || [ "$JAVA_VER" -eq 25 ]; then
        return 0
    else
        return 1
    fi
...
checkParallelGCConditions() {
    ...
    JAVA_VER=$(getJavaVersion)
    if [ "$JAVA_VER" -ne 17 ] && [ "$JAVA_VER" -ne 21 ] && [ "$JAVA_VER" -ne 25 ]; then
        return 2
    fi
...
getParallelGCFailReason() {
    ...
    if [ "$JAVA_VER" -ne 17 ] && [ "$JAVA_VER" -ne 21 ] && [ "$JAVA_VER" -ne 25 ]; then
        echo "Java $JAVA_VER detected (requires 17, 21, or 25)"
...
removeInternalKeystore() {
    if [ "$SOURCE" == "MorpheApp" ] || [ "$SOURCE" == "Wchill-patcheddit" ] || [ "$SOURCE" == "RVX-Morphed" ] || [ "$SOURCE" == "Anddea-Morphed" ] || [ "$SOURCE" == "Anddea" ] || [ "$SOURCE" == "De-ReVanced" ]; then
...
patchApp() {
    ...
    if isLowMemory; then
        ...
        GC_OVERRIDE_MSG="Low memory detected (${AVAILABLE_MEM}MB < ${MIN_HEAP}MB) - Forced Serial GC for stability"
    ...
    local DEVICE_ARCH LIB_OPTIMIZE_ARGS=() LIB_OPTIMIZE_INFO="Disabled" LIB_OPTIMIZE_METHOD="None"
...
║ GC Type             : $GC_INFO
║ GC Mode             : $GC_MODE
║ Low Memory Override : $LOW_MEM_OVERRIDE
...
║ Supports --striplibs   : $SUPPORTS_STRIP_LIBS
╠═══════════════════════════════════════════════════════════════╣
║ BUILD OPTIONS
╠═══════════════════════════════════════════════════════════════╣
║ Source                    : $SOURCE
...
╠═══════════════════════════════════════════════════════════════╣
$(if [ ${#LIB_OPTIMIZE_ARGS[@]} -gt 0 ]; then printf '║ %s\n' "${LIB_OPTIMIZE_ARGS[@]}"; else echo "║ None"; fi)
╠═══════════════════════════════════════════════════════════════╣
║ SIGNING ARGUMENTS
╠═══════════════════════════════════════════════════════════════╣
$(printf '║ %s\n' "${SIGNING_ARGS[@]}")
...
EOF
    ...
    java \
        ...
        "${DIALOG[@]}" \
            ...
            --cursor-off-label \
            --programbox "Patching $APP_NAME $APP_VER [$FORMAT_INFO | JDK $JAVA_VER | $GC_MODE | ${HEAP_SIZE}MB]" -1 -1
