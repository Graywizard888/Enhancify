...
SUPPORTED_EXTENSIONS=("apk" "apkm" "xapk")
...
patchApp() {
    ...
    local DEVICE_ARCH
    local RIP_LIBS_ARGS=()
    local RIP_LIBS_INFO="Disabled"
    
    DEVICE_ARCH=$(getDeviceArch)
    
    if isMorpheAppSource; then
        RIP_LIBS_ARGS=()
        RIP_LIBS_INFO="Disabled ( Incompatible MorpheApp source)"
    elif shouldRipLibs; then
        readarray -t RIP_LIBS_ARGS < <(getRipLibsArgs "$DEVICE_ARCH")
        RIP_LIBS_INFO="CLI Overrided $DEVICE_ARCH"
    else
        RIP_LIBS_INFO="Used Optimize Libs $DEVICE_ARCH"
    fi
    ...
    if isMorpheAppSource; then
        ...
        SIGNING_INFO="CLI keystore ( Incompatible MorpheApp source)"
    elif hasCustomKeystore; then
...
╠═════════════════════════════════╣
║ Source               : $SOURCE
║ CLI Riplib/Antisplit : ${CLI_RIPLIB_ANTISPLIT:-off}
