...
get_github_token() {
    local token_file="Enhancify/github_token.json"
    if [ -f "$token_file" ]; then
        jq -r '.token' "$token_file" 2>/dev/null
    fi
}
...
downloadFile() {
    ...
    if [ "$DISABLE_NETWORK_ACCELERATION" != "on" ]; then
        ...
        (
            ...
            local aria2c_cmd=(aria2c --console-log-level=warn --summary-interval=1 --download-result=hide \
                   --out="$file" \
                   --split=4 --min-split-size=5M --max-connection-per-server=4 \
                   --file-allocation=none)

            if [ -n "$GITHUB_TOKEN" ]; then
                aria2c_cmd+=(--header="Authorization: token $GITHUB_TOKEN")
            fi
        ...
        (
            local wget_cmd=("${WGET[@]}" "$url" -O "$output_file")

            if [ -n "$GITHUB_TOKEN" ]; then
                wget_cmd+=(--header="Authorization: token $GITHUB_TOKEN")
            fi
...
fetchAssetsInfo() {
    ...
    if [ -n "$GITHUB_TOKEN" ]; then
        CURL_CMD+=(--header "Authorization: token $GITHUB_TOKEN")
    fi
    ...
    if [ "$("${CURL_CMD[@]}" "https://api.github.com/rate_limit" | jq -r '.resources.core.remaining')" -gt 2 ]; then
        ...
        if [ -n "$VERSION_URL" ]; then
            if VERSION=$("${CURL_CMD[@]}" "$VERSION_URL" | jq -r '.version' 2> /dev/null); then
        ...
        if ! "${CURL_CMD[@]}" "$PATCHES_API_URL" | jq -r '
                if type == "array" then .[0] else . end |
