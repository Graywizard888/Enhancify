bundleParser() {
    ...
    while true; do
        ...
        case "$choice" in
            1)
                ...
                url_source_name="${url_source_name%-patches-bundle}"
    ...
    if [ "$bundle_mode" == "file" ]; then
        ...
        source_name="${source_name%-patches-bundle}"
    ...
    for var in $(compgen -v | grep "^ASSET_URL_"); do
        ...
        CTR=3
        while [ "$asset_size" != "$(stat -c %s "$asset_file" 2>/dev/null)" ]; do
            [ $CTR -eq 0 ] && notify msg "Oops! Unable to download $asset_name completely.\n\nRetry or change your Network." && return 1
            ((CTR--))

            local asset_text="Source  : $SOURCE\n"
            asset_text+="File    : $asset_name\n"
